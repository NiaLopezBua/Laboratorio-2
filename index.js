function validateName () {
    var nameField = document.getElementById ('name');
    var validName = /^[A-Za-z\s]+$/;
    var errorName = document.getElementById ('nameError')

    if (nameField.value == 0) {
        errorName.innerHTML = "El campo es obligatorio.";
        nameField.classList.add ('invalid');
        nameField.classList.add ('errorIcon');
        nameField.classList.remove ('valid');
        nameField.classList.remove ('successIcon');
        return false;
    }

    else if (validName.test(nameField.value)) {
        nameField.classList.remove('invalid');
        nameField.classList.remove('errorIcon');
        errorName.innerHTML ='';
        nameField.classList.add('valid');
        nameField.classList.add('successIcon');
        return true;
    }

    else{
        errorName.innerHTML = "El nombre no es válido.";
        nameField.classList.add('invalid');
        nameField.classList.add('errorIcon');
        nameField.classList.remove('valid');
        nameField.classList.remove('successIcon');
        return false;
    }

}

function validatePassword(){
    var passwordField = document.getElementById('password');
    var validPassword = /^[\s\S]{8,15}$/;
    var errorPassword = document.getElementById('passError');

    if (passwordField.value == 0){
        errorPassword.innerHTML = "El campo es obligatorio.";
        passwordField.classList.add("invalid");
        passwordField.classList.add("errorIcon");
        passwordField.classList.remove("valid");
        passwordField.classList.remove("successIcon");
        return false;
    }

    else if(validPassword.test(passwordField.value)){
        passwordField.classList.remove("invalid");
        passwordField.classList.remove("errorIcon");
        errorPassword.innerHTML = "";
        passwordField.classList.add("valid");
        passwordField.classList.add("successIcon");
        return true;
    }

    else{
        errorPassword.innerHTML = "Debe contener al menos 8 caracteres";
        passwordField.classList.add("invalid");
        passwordField.classList.add("errorIcon");
        passwordField.classList.remove("valid");
        passwordField.classList.remove("successIcon");
	return false;
    }
}

function validateConfirm(){
    var passwordField = document.getElementById('password');
    var confirmField = document.getElementById('confirm');
    var errorConfirm = document.getElementById('confirmError');

    if (confirmField.value == 0) {
        errorConfirm.innerHTML = "El campo es obligatorio.";
        confirmField.classList.add ("invalid");
        confirmField.classList.add ("errorIcon");
        confirmField.classList.remove ("valid");
        confirmField.classList.remove ("successIcon");
        return false;
    }

    else if(passwordField.value == confirmField.value){
        confirmField.classList.remove ("invalid");
        confirmField.classList.remove ("errorIcon");
        errorConfirm.innerHTML = "";
        confirmField.classList.add ("valid");
        confirmField.classList.add ("successIcon");
        return true;
    }

    else{
        errorConfirm.innerHTML = "Las contraseñas no coinciden";
        confirmField.classList.add ("invalid");
        confirmField.classList.add ("errorIcon");
        confirmField.classList.remove ("valid");
        confirmField.classList.remove ("successIcon");
        return false;
    }
}


function validateTel(){
    var telField = document.getElementById('tel');
	var validateTel = /^(\()?\d{3}(\))?(-|\s)?\d{3}(-|\s)\d{4}$/;
	var errorTel = document.getElementById('telError');
    
    if(telField.value == 0){
        telField.innerHTML = 'El campo es obligatorio.';
        telField.classList.add('invalid');
        telField.classList.add('errorIcon');
        telField.classList.remove('valid');
        telField.classList.remove('successIcon');
        return false;

    }
    else if (validateTel.test(telField.value)) {
        telField.classList.remove ("invalid");
        telField.classList.remove ("errorIcon");
        errorTel.innerHTML ="";
       telField.classList.add ("valid");
        telField.classList.add ("successIcon");
        return true;
    }

    else {
        errorTel.innerHTML = "El teléfono no es válido.";
        telField.classList.add ("invalid");
        teñField.classList.add ("errorIcon");
        telField.classList.remove ("valid");
        telField.classList.remove ("successIcon");
        return false;
    }

}

function validateEmail(){
	var emailField = document.getElementById('email');
	var validEmail = /^\w+([.-_+]?\w+)*@\w+([.-]?\w+)*(\.\w{2,10})+$/;
	var errorEmail = document.getElementById('emailError');

	if(emailField.value == 0){
		errorEmail.innerHTML = "El campo es obligatorio.";
		emailField.classList.add('invalid');
		emailField.classList.add('errorIcon');
        emailField.classList.remove('valid');
        emailField.classList.remove('successIcon');
		return false; 
	}

	else if(validEmail.test(emailField.value)){
		emailField.classList.remove('invalid');
		emailField.classList.remove('errorIcon');
		errorEmail.innerHTML = '';
		emailField.classList.add('valid');
		emailField.classList.add('successIcon');
		return true; 
	}
    
	else{
		errorEmail.innerHTML = "El email no es válido.";
		emailField.classList.add("invalid");
		emailField.classList.add("errorIcon");
        emailField.classList.remove("valid");
        emailField.classList.remove("successIcon");
		return false; 
	}
} 

function borderButton(){
	var borderButton = document.getElementById('button');
	    borderButton.onmouseover = () => {
		borderButton.classList.add('borderColor'); 	}
	    borderButton.onmouseout = () => {	
		borderButton.classList.remove('borderColor'); 	}
}

borderButton()

function validateForm() {
    if (validateConfirm () && validatePassword () && validateEmail () && validateName ()) {
        alert ("La inscripción ha sido correcta.");
        return true; 
    }
    
    else { 
        return false;
    }
    }
